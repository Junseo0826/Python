...
print(s[0])
print(s[6:10])  # 6 ~ 9
print(s[:-2])   # 0 ~ 7 (뒤에서 두번째 앞까지)

# 문자열 분리 : split()
s = 'Welcome to Python'
print(s.split())            # 공백을 기준으로 분리, 리스트 형식으로 분리
...
print(s.strip())
print([x.strip() for x in s.split(',')])    # s를 먼저 ','를 기준으로 분리하고 하나씩 x에 담는다. 그 후에 x에 있는 공백을 제거한다.
...
print('-'.join('010.1234.5678'.split('.'))) # '.'으로 구분된 번호를 '-'로 붙인다.
